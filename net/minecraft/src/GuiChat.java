...
public class GuiChat extends GuiScreen
{
    ...
    protected void keyTyped(char par1, int par2)
    {
        ...
        else if (par2 == 28)
        {
            ...
            {
            	//TODO Not really a todo, but to mark where the processing is. :3
            	if(mod_ChatMacros.macrosEnabled){
            	String msg = mod_ChatMacros.messWithMessage(var3);
            	if(msg != null)
                mc.thePlayer.sendChatMessage(mod_ChatMacros.messWithMessage(var3));
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
        ...
        drawRect(2, this.height - 37, fontRenderer.getStringWidth("Finish Mode: Close Chat Pane.") + 4, this.height - 16, Integer.MIN_VALUE);
        this.inputField.drawTextBox();
        if(mod_ChatMacros.macrosEnabled)
        drawString(fontRenderer, "§9Chat Macros §aEnabled." , 2, height-36, 0xFFFFFF);
        else drawString(fontRenderer, "§9Chat Macros §cDisabled." , 2, height-36, 0xFFFFFF);
        if(mod_ChatMacros.closeOnFinish)
        drawString(fontRenderer, "§9Finish Mode: Close Chat Pane." , 2, height-26, 0xFFFFFF);
        else drawString(fontRenderer, "§9Finish Mode: Clear Message." , 2, height-26, 0xFFFFFF);
